...
final class TraceEntry
{
    ...
    {
        if (isset($this->entry['class']) && stringStartsWith(VarDump::_CLASS_ANON, $this->entry['class'])) {
    ...
    {
        $entryFile = stringReplaceFirst(VarDump::_CLASS_ANON, '', $this->entry['class']);
        $this->entry['file'] = substr($entryFile, 0, strpos($entryFile, '.php') + 4);
        $this->entry['class'] = VarDump::_CLASS_ANON;
