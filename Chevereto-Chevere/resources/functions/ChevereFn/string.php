...
function stringStartsWithNumeric(string $haystack): bool
{
 ...
 */
function stringCompare(string $safe, string $user): bool
{
    $safe .= chr(0);
    $user .= chr(0);
    $safeLen = strlen($safe);
 ...
 */
function stringTruncate(string $string, int $limit, string $pad = '...'): string
{
    $encoding = 'UTF-8';
 ...
 */
function stripNonAlphanumerics(string $string): string
{
 ...
 */
function stringStripWhitespace(string $string): string
{
 ...
 */
function stringStripExtraWhitespace(string $string): string
{
    return preg_replace('/\s+/', ' ', $string) ?? '';
}
